...
const portalIconMap: Record<PortalIconName, LucideIcon> = {
  hub: Bot,
  projects: Rocket,
  knowledge: BookOpen,
  apps: Bot,
          ...
          >
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              ...
              >
               ...
               <a href={siteLinks.knowledgeBase} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-2 py-3 text-sm font-bold text-slate-400 hover:text-white">
                Knowledge Base
            ...
            <div className="grid content-start gap-4">
              ...
              <aside className="rounded-2xl border border-emerald-400/10 bg-emerald-500/[0.04] p-5">
                <p className="font-mono text-[10px] font-bold tracking-[0.18em] text-emerald-300">CURRENT WORK</p>
                 <h3 className="mt-4 font-display text-xl font-black text-white">{currentFocus.title}</h3>
                 <dl className="mt-5 grid gap-3 text-xs">
                   <div className="flex justify-between gap-4"><dt className="text-slate-500">Updated</dt><dd className="text-slate-300">{currentFocus.updated}</dd></div>
                   <div className="flex justify-between gap-4"><dt className="text-slate-500">Source</dt><dd className="text-slate-300">{currentFocus.source}</dd></div>
                   <div className="flex justify-between gap-4"><dt className="text-slate-500">Publish</dt><dd className="text-emerald-300">{currentFocus.publish}</dd></div>
                 </dl>
                 <a href={currentFocus.docs} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-xs font-bold text-white">
                  Open Documentation
                  <ArrowRight className="h-3.5 w-3.5" aria-hidden="true" />
                </a>
              </aside>
        ...
        <section id="knowledge" className="scroll-mt-24 border-y border-white/[0.06] bg-[#080c1a]/80">
          <div className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
            <SectionHeading label="02 / KNOWLEDGE" title="Knowledge Base" description="読むものはすべてGitHub Pagesへ。AIと人が同じ知識へ戻れる場所です。" />
             <a href={siteLinks.knowledgeBase} target="_blank" rel="noreferrer" className="group mb-4 flex flex-col justify-between gap-8 rounded-3xl border border-sky-400/15 bg-gradient-to-br from-sky-500/10 via-[#0a1023] to-violet-500/10 p-7 sm:flex-row sm:items-end sm:p-9">
              <div>
      ...
      <footer className="relative z-10 border-t border-white/[0.06]">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-5">
             <a href={siteLinks.githubProfile} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-white">
              <Github className="h-4 w-4" aria-hidden="true" /> GitHub
            </a>
             <a href={siteLinks.xProfile} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-white">
